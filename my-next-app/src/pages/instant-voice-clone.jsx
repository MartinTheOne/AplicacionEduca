...
function InstantVoiceClone({ setDisabled }) {
  ...
  useEffect(() => {
    const getVoices = async () => {
      const voicesLocalStr = localStorage.getItem("voices");
      if (voicesLocalStr) {
        setVoices(JSON.parse(voicesLocalStr));
      } else {
        const data =await fetch("/api/get-voices");
        if (data.ok){
  ...
  const VerifyName = () => {
    const searchName = voices.find((n) =>
  ...
  const handleSubmit = async (e) => {
    ...
    if(VerifyName())return notyf.error("El nombre ya esta en uso")
    setDisabled(true)
